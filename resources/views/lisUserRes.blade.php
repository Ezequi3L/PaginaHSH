       ...
       <img src= <?php if ($foto != null){ $src = $foto->src; echo '"'; echo $src; echo '"';} else{echo '"'; echo $imgnodisp; echo '"';} ?>>
        <div class="card-body">
          ...
          <center> <form action="{{ route('deleteReserva', [$reserva]) }}" method="POST">
              {{ csrf_field() }}
              {{ method_field('DELETE') }}
              <button type="submit" onclick=<?php if(((Carbon::now())>(Carbon::createFromDate($reserva->fecha)->subMonths(2))) or ($reserva->hotsale)) {?> "return confirm('No se te devolverá una semana al cancelar esta reserva ¿Estás seguro que deseas cancelarla?');"<?php } else {?>"return confirm('¿Estás seguro que deseas cancelar esta reserva?')"<?php } ?> class="btn btn-sm btn-outline-danger">Cancelar Reserva</button>
           </form>
          </center>
        </div>
      </div>
